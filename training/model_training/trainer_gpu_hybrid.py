...
def trainer_gpu_hybrid(model, normalizer, model_type, train_loader, val_loader, hyperparameters, OUTDIR, gpu_num, train_eval_loader=None, contrastive_weight=1.0):
    """Hybrid GPU trainer: DGL graphs on CPU, tensor computations on GPU"""
    ...
    if model_type == "Painn":
        ...
        best_model = train_alignn_gpu_hybrid(model, normalizer, train_loader, val_loader, hyperparameters, OUTDIR, gpu_num)
    else:
...
def train_alignn_gpu_hybrid(model, normalizer, train_loader, val_loader, hyperparameters, OUTDIR, gpu_num):
    """Full GPU training: Everything on GPU including DGL graphs"""
    ...
    print("Using full GPU approach: Everything on GPU including DGL graphs")
    ...
    # Move model buffers to GPU as well
    ...
    for epoch in range(max_epochs):
        ...
        for j, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            # Move everything to GPU including DGL graphs
            try:
        ...
        # Evaluation
        predictions, targets, train_avg_loss = evaluate_model_hybrid(model, normalizer, "ALIGNN", train_loader, loss_fn, gpu_num)
        predictions, targets, valid_avg_loss = evaluate_model_hybrid(model, normalizer, "ALIGNN", val_loader, loss_fn, gpu_num)
...
def evaluate_model_hybrid(model, normalizer, model_type, dataloader, loss_fn, gpu_num, is_contrastive=False, contrastive_weight=1.0):
    """Full GPU evaluation: Everything on GPU including DGL graphs"""
    try:
    ...
    if model_type == "Painn":
        ...
        with torch.no_grad():
            for j, d in enumerate(dataloader):
                if model_type == "CGCNN":
                    ...
                    try:
                        ...
                        print(f"Error moving data to GPU during evaluation: {e}")
                        print("Falling back to CPU")
                        device = torch.device("cpu")
                        model = model.to(device)
                        graph = graph.to(device)
            ...
            return predictions, targets, loss_output
